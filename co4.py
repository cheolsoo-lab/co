...
def render_advanced_macro_weather_panel():
  ...
  with st.spinner(
      '비트코인 멀티 타임프레임 및 세력 수급(매집/설거지) 분석 중...'
  ):
...
# 🌟 최상단에 안정화된 거시 유동성 및 마켓 날씨 판넬 추가
...
if not df_all.empty and exchange is not None:
  ...
  with col_btn1:
    if st.button(
        '🎯 1번 분석 실행 (패턴 검증)', use_container_width=True, type='primary'
    ):
      with st.spinner(
          '⏳ [1번] 전체 대상 패턴 검증 엔진 가동 중 (다소 시간 소요)...'
      ):
